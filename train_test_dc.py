...
def train(model, epoch, data, optimizer, para, single_data):
    model.train()
    train_loss = 0
    for batch_idx, (input_data, target) in enumerate(data):
        ...
        loss_vae = vae_loss(recon, input_data, u, logvar, img_size=3 * 512 * 512)
        loss_o = o_new_loss(u, target, model)
        loss = para.alpha * loss_vae + para.beta * loss_o
        ...
        if batch_idx % 200 == 0:
            if batch_idx > 0:
                refresh(model, single_data, para)
...
def main_trainer(model_name, epoch_number, labels_list, para, continue_train=False, model_path=''):
    data_name = 'dog-cat'
    ...
    if continue_train is not False:
        ...
        model = ClusterVAENet(backbone=model_name, channels_img=3, img_size=512, cls_list=labels_list, device=para.device)
        model = model.to(para.device)
    train_dataset, test_dataset, _ = get_dataset(batch_size=para.batch_size, is_contrastive=False)
    single_train_dataset, _, _ = get_dataset(batch_size=1, is_contrastive=False)
...
# 3 -> 28 reverse-padding: 1,0,1
# 64 -> 512 reverse-padding: 1,1,1
